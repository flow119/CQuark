                ...
                if(expend == expbegin) {//simple
                    if(tlist[expbegin].type == TokenType.KEYWORD) {
                        if(tlist[expbegin].text == "return") {
                            ...
                            CQ_Value cq_Value = new CQ_Value();
                            cq_Value.m_type = typeof(bool);
                            cq_Value.value = true;
                            CQ_Expression_Value_Value<bool> subvalue = new CQ_Expression_Value_Value<bool>(cq_Value);
                            values.Add(subvalue);
                        }
                        else if(tlist[expbegin].text == "false") {
                            CQ_Value cq_Value = new CQ_Value();
                            cq_Value.m_type = typeof(bool);
                            cq_Value.value = false;
                            CQ_Expression_Value_Value<bool> subvalue = new CQ_Expression_Value_Value<bool>(cq_Value);
                            values.Add(subvalue);
                        }
